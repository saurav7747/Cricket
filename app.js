...
function addBall(ball) {
  ...
  // 2nd innings: check if target has been reached
  if (STATE.match.currentInnings === 1 && inn.target && inn.runs >= inn.target) {
    autoSave();
    renderScoreboard();
    endInnings();
    return;
  ...
  // Check innings end (all out / overs up)
  checkInningsEnd();
  autoSave();
  renderScoreboard();
}
...
function showWicketModal() {
  ...
  // Players not yet out and not currently at the crease
  const remainingBatsmen = inn._battingPlayers.filter(p =>
    !inn.batsmen[p] ||
    (inn.batsmen[p] && !inn.batsmen[p].out && p !== inn.onStrikeBatsman && p !== inn.nonStrikeBatsman)
  );

  const noNextBatsman = remainingBatsmen.length === 0;
  const nextList = document.getElementById('modal-next-batsman-list');
  nextList.innerHTML = noNextBatsman
    ? '<div style="padding:12px 12px;color:var(--text-2);font-size:13px;">No more batsmen — innings will end after this wicket</div>'
  ...
  // Both selections required when a next batsman exists; only dismissed player needed when all out
  function tryConfirmWicket() {
    if (!outPlayer) return;
    if (!noNextBatsman && !nextPlayer) return; // wait for next batsman pick
    hideModal('wicket');
...
function processWicket(outPlayer, nextPlayer) {
  ...
  inn.wickets++;
  inn.balls++;
  if (bowler) { bowler.wickets++; bowler.ballsThisOver++; }
  ...
  // Seat the new batsman in the dismissed player's slot
  if (outPlayer === inn.onStrikeBatsman) {
    inn.onStrikeBatsman = nextPlayer || null;
    if (nextPlayer && !inn.batsmen[nextPlayer]) inn.batsmen[nextPlayer] = createBatsman(nextPlayer);
  } else {
    inn.nonStrikeBatsman = nextPlayer || null;
    if (nextPlayer && !inn.batsmen[nextPlayer]) inn.batsmen[nextPlayer] = createBatsman(nextPlayer);
  }

  // Check if the over is now complete (ball landed on over boundary)
  if (inn.balls > 0 && inn.balls % 6 === 0) {
    completeOver();
    return;
  }

  // Check innings-end conditions AFTER placing the new batsman
  // (10 wickets = all out; overs exhausted; genuinely no batsmen left)
  checkInningsEnd();
  autoSave();
  renderScoreboard();
}
...
function checkInningsEnd() {
  ...
  // All out = 10 wickets fallen, regardless of squad size
  const allOut = inn.wickets >= 10;
  const oversUp = inn.balls >= maxBalls;
  // Only end on missing batsman if wickets are also exhausted (not mid-wicket replacement)
  const noOnePlaying = !inn.onStrikeBatsman && !inn.nonStrikeBatsman;

  if (allOut || oversUp || noOnePlaying) {
    endInnings();
  }
}
